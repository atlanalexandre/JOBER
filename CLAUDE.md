...
| Application installée non fermée | Le service worker met à jour les fichiers, mais l'onglet ouvert continue d'exécuter le JavaScript du premier jour. Le back-office affichait encore, le 27/08/2026, un bouton supprimé le 14/08. Le build écrit `dist/version.json` et injecte `__BUILD_ID__` : `src/lib/version.js` compare les deux et propose un rechargement. **Ne jamais recharger d'autorité** — cela ferait perdre une réservation en cours. Et avant de conclure qu'un correctif ne marche pas : vérifier que la version testée est bien la version déployée |
| Appellation courante ≠ libellé | « Femme de ménage » ne renvoyait rien, alors que c'est le terme le plus employé du premier secteur de la plateforme. Idem « enseignant », « vigile ». `ALIAS_METIERS` dans `src/constants/data.js` rattache ces mots à leur métier. **On rapproche, on n'élargit pas** : un mot dont le métier n'existe pas — « nounou », « plombier » — doit rester sans résultat, et c'est testé |
| Écriture inclusive et recherche | Vingt métiers portent une terminaison entre parenthèses — « Gouvernant(e) d'étage », « Hôte(sse) de caisse ». Un `includes` sur le libellé brut ne trouve **ni** « gouvernant » **ni** « gouvernante ». Toute recherche sur un libellé métier passe par `correspondRecherche()` de `src/constants/data.js`, jamais par `toLowerCase().includes()` |
| Montant affiché | Jamais `${x} €` ni `x.toFixed(2)` dans un texte : « 20.2 € », « 29.99 €/mois » et « 11.5 – 13.5 €/h » s'affichaient encore le 25/09/2026. Côté écran `formatMontant()` (`src/constants/plans.js`), côté e-mail et notification `euros()` (`api/_email.js`) — « 20,20 € » |
