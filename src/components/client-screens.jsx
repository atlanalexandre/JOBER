...
Signé électroniquement le ${new Date().toLocaleDateString("fr-FR")}`}
      ...
      <div style={{ padding:"22px 18px" }}>
        ...
        {/* Les blocs « étape 2 » et « étape 3 » existaient EN DOUBLE dans ce
            composant. Le premier exemplaire de l'étape 3 contenait, sous le
            récapitulatif, une copie entière du formulaire de l'étape 1 — type de
            prestation, dates, durée, description et un second bouton
            « Continuer » qui renvoyait à l'étape 2. L'écran de paiement affichait
            donc : récapitulatif, puis le formulaire de réservation, puis à nouveau
            le récapitulatif, puis le paiement.

            Les deux exemplaires de l'étape 2 étaient identiques ; ceux de l'étape 3
            avaient déjà divergé (bloc « Mode Urgence actif » d'un côté, note
            « Prestation urgente » de l'autre). Supprimé le 16/08/2026. */}
...
export function FAQScreen({ onBack, role }) {
  ...
  const faqs = role === "prestataire" ? [
    ...
    { q:"Comment annuler une réservation ?", a:"Vous pouvez annuler une prestation depuis votre espace client. Les frais de service restent dus, car ils couvrent des coûts déjà engagés. Le montant de la prestation vous est remboursé intégralement tant qu'elle n'a pas commencé. En revanche, si vous annulez après le début ou ne vous présentez pas, le montant intégral reste dû au prestataire (CGPS art. 8.1). Si c'est le prestataire qui annule, vous êtes remboursé de tout, frais de service compris." },
