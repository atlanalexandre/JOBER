...
// production sur alane.fr. Sert à masquer les outils réservés aux essais —
// le serveur, lui, les refuse en production quoi qu'affiche l'écran.
const estRecette = typeof window !== "undefined"
  && (/\.vercel\.app$/.test(window.location.hostname) || ["localhost", "127.0.0.1"].includes(window.location.hostname));
...
// ═══════════════════════════════════════════════════════════════════════════
// LES FAUX DOCUMENTS OFFICIELS ONT ÉTÉ SUPPRIMÉS — 24/08/2026
// ═══════════════════════════════════════════════════════════════════════════
//
// `DemoDocPreview` fabriquait sept documents à l'écran pour les comptes de
// démonstration : un extrait Kbis sous en-tête « TRIBUNAL DE COMMERCE DE
// PARIS », une attestation « URSSAF ÎLE-DE-FRANCE » avec son numéro de
// référence, un relevé d'identité bancaire « CRÉDIT AGRICOLE ILE-DE-FRANCE »,
// une carte d'identité « RÉPUBLIQUE FRANÇAISE », une attestation d'assurance,
// un justificatif de domicile et une photo.
//
// Trois raisons de les retirer, par ordre de gravité :
//
// 1. Ce sont des REPRODUCTIONS DE DOCUMENTS OFFICIELS ET BANCAIRES, complètes
//    avec en-têtes, numéros et tampons « EN VIGUEUR ». Elles n'ont jamais servi
//    qu'à peupler un écran de démonstration, mais rien dans le produit ne les
//    distinguait d'un vrai document déposé par un prestataire : même modale,
//    même cadre, même bouton. Une capture d'écran suffit à les sortir de leur
//    contexte.
//
// 2. Le Kbis affichait le code APE « 7820Z — Activité des agences de travail
//    temporaire ». C'est exactement la qualification que l'article 4.1 des
//    CGPS s'emploie à écarter, et qu'un contrôle URSSAF cherche en premier.
//
// 3. Ils portaient une identité de société inventée — « ATLAN SERVICES SAS »,
//    SIRET « 823 456 789 00015 », « 12 Rue de la Paix, 75002 Paris » — qui n'a
//    plus lieu d'être le jour où la vraie société existera.
//
// Les comptes de démonstration affichent désormais un simple encart qui dit ce
// qu'il est. Un écran de démonstration doit s'annoncer comme tel, pas imiter
// une administration.
function DemoDocPreview({ type }) {
  ...
  return (
    <div style={{ background:"rgba(255,255,255,0.04)", border:"1px dashed rgba(255,255,255,0.25)", borderRadius:12, padding:"36px 28px", maxWidth:460, margin:"0 auto", textAlign:"center" }}>
      <div style={{ fontSize:40, marginBottom:14 }}>🧪</div>
      <div style={{ color:"#F0F0F5", fontWeight:800, fontSize:15, marginBottom:8 }}>
        {LIBELLE[type] || "Document"} — compte de démonstration
      </div>
      <div style={{ color:"#9BA0BA", fontSize:13, lineHeight:1.7 }}>
        Ce compte sert aux essais : aucun document n'a été déposé, et il n'y a rien à vérifier ici.
        Les documents réels s'affichent tels qu'ils ont été envoyés par le prestataire.
      </div>
    </div>
  );
}
          ...
          <div onClick={()=>setDocModal(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.82)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:1500, padding:"0" }}>
            <div onClick={e=>e.stopPropagation()} style={{ background:"#0D1B3E", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:600, maxHeight:"88vh", display:"flex", flexDirection:"column", border:"1px solid rgba(255,255,255,0.1)", borderBottom:"none" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 18px", borderBottom:"1px solid rgba(255,255,255,0.08)", flexShrink:0 }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  {/* Recette uniquement : le serveur refuse en production (bo-action seed_docs). */}
                  {estRecette && <button onClick={async()=>{
